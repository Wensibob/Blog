	...
	if(xmlHttp.readyState == 4){
		if(xmlHttp.status == 200){
			...
			if(text == "true"){	// 用户id已经存在了
				document.getElementById("userid_msg").innerHTML = "用户名重复，无法使用！" ;
				flag = false ;
			} 
			else {
				document.getElementById("userid_msg").innerHTML = "此用户名可以注册！" ;
				flag = true ;
			}
		}
	}
...
function checkNameEmpty(name) {
	if (name==""||name==null) {
		document.getElementById("name_msg").innerHTML="姓名不能为空";
		flag=false;
	}else{
